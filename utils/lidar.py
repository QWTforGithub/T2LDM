    ...
    def sample_to_lidar(
        ...
        if(upsampling is not  None):
            ...
            for i in range(len(upsampling)):
                self.save_pointcloud(xyz=upsampling[i], colors=None, name=f"{upsampling_dir}/sparse_rank_{rank}_dataset_{dataset}_step_{num_step}_sample_{num_sample}_batch_{i}.ply")
        ...
        if(downsampling is not  None):
            ...
            for i in range(len(downsampling)):
                self.save_pointcloud(xyz=downsampling[i], colors=None, name=f"{downsampling_dir}/dense_rank_{rank}_dataset_{dataset}_step_{num_step}_sample_{num_sample}_batch_{i}.ply")
