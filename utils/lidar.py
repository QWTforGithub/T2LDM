...
class LiDARUtility(nn.Module):
    ...
    def sample_to_lidar(
        ...
        if(semantic is not None):
            ...
            self.save_semantic(semantic=semantic, name=f"{semantic_data}/semantic_rank_{rank}_dataset_{dataset}_step_{num_step}_sample_{num_sample}.npy")
            ...
            for i,labels in enumerate(semantic):
                ...
                self.save_pointcloud(xyz=xyz[i], colors=gt_color, name=f"{xyzcolor_dir}/xyzcolor_rank_{rank}_dataset_{dataset}_step_{num_step}_sample_{num_sample}_batch_{i}.ply")
                self.save_pointcloud(xyz=generation[i], colors=gt_color, name=f"{generationcolor}/generationcolor_rank_{rank}_dataset_{dataset}_step_{num_step}_sample_{num_sample}_batch_{i}.ply")
