...
class TrainingConfig:
    ...
    data_root: str = "/root/dataset/SemanticKITTI/dataset/sequences"
    aug: tuple[int, int] = ("-", "-") # ("rotation", "flip")
    resolution: tuple[int, int] = (64, 1024)
    depth_range: tuple[int, int] = (1.45, 80.0) # (1.45, 80.0) # (0.01, 50.0)
    fov: tuple[int, int] = (3, -25)
    batch_size_train: int = 8 # 2 # 16
    batch_size_eval: int = 4
    num_workers: int = 8 # 2 # 16
    ...
    pretrained_checkpoint_dir: str = "/root/models/checkpoints/unconditional_semantickitti/unconditional_semanticKITTI_full_training_scrg/4_3090_700000.pth"
    # ---- training setting ----
