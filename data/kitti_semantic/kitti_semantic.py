...
class KITTISemanticDataset(Dataset):
    ...
    def __init__(
            self,
            data_root="/ihoment/youjie10/qwt/dataset/SemanticKITTI/dataset/sequences",
            training=True,
            aug=["rotation", "flip"],
        ...
        if(text_path is None):
            ...
            for info in infos:
                bin_paths.append(Path(info["bin_path"]))
                semantic_paths.append(Path(info["semantic_path"]))
    ...
    def __getitem__(self, idx):
        ...
        text = self.texts[idx] if(self.text_path is not None) else ""
