...
ROOT_PATH = "/root/dataset/rsd_data/nuscenes/v1.0-trainval"
...
def get_folder_size(folder_path):
    ...
    for dirpath, dirnames, filenames in os.walk(folder_path):
        for f in filenames:
            ...
            if not os.path.islink(fp):  # 忽略符号链接
                total_size += os.path.getsize(fp)
    return total_size

def copy_train_val_bin(
        dest_folder="/root/dataset/rsd_data/nuscenes/v1.0-trainval/samples/LIDAR_TOP_train_val"
):

    infos = read_pkl()
    for i,info in tqdm(enumerate(infos), total=len(infos)):
        lidar_path = f"{ROOT_PATH}/{info['lidar_path']}"
        lidar_name = lidar_path.split("/")[-1]
        dest_path = f"{dest_folder}/{lidar_name}"
        shutil.copy(lidar_path, dest_path)
def get_color():
    colors = [
