...
def check_semantic_object(semantic, scene=True):
    indexes = []
    if(scene):
        for scene_index in SCENE_INDEX:
            if(semantic.__contains__(scene_index)):
                indexes.append(scene_index)
    else:
        for object_index in OBJECT_INDEX:
            if(semantic.__contains__(object_index)):
...
def text_quantity_combination_l1(item, object="car", threshold=5):
    gt_names = item["gt_names"]
    if (len(gt_names) > 0):
        gt_names = gt_names[gt_names != "ignore"]
    gt_boxes = item["gt_boxes"]
    ...
    if(not object in gt_names.tolist()):
        # text1 = f"No {object}{suffix}"
        return None
    else:
        text1 = text_quantity_l1(item, object, threshold)[:-1]
        gt_names = gt_names[gt_names != object]

    if(len(gt_names) > 0):
        other_object = random.choice(gt_names)
        text2 = text_quantity_l1(item, other_object, threshold=2)[:-1].lower()

    else:
        other_object = random.choice(OBJECT_NAME)
        other_object_suffix = "s"
        if (other_object == "bus"):
            other_object_suffix = "es"
        text2 = f"no {other_object}{other_object_suffix}"

    change_i = random.choice([True,False])
    if(change_i):
        temp = text1
        text1 = text2.capitalize()
        text2 = temp.lower()

    text = f"{text1} and {text2}."
    return text

def text_quantity_combination_l2(item, object="car", threshold=5, random_change=False):
    gt_names = item["gt_names"]
    if (len(gt_names) > 0):
        gt_names = gt_names[gt_names != "ignore"]
    gt_boxes = item["gt_boxes"]
    ...
    text2 = random.choice(gt_names)

    other_object_suffix = "s"
    if (text2 == "bus"):
        other_object_suffix = "es"

    text2 = f"{text2}{other_object_suffix}"
...
def text_contain_l1(item,object="car",random_change=False):
    gt_names = item["gt_names"]
    if(len(gt_names) > 0):
        gt_names = gt_names[gt_names != "ignore"]
    gt_boxes = item["gt_boxes"]
    ...
    if (not object in gt_names.tolist()):
        # object_name = f"no {object}{suffix}"
        return None
    else:
        object_name = f"one {object}"

    if(len(gt_names) == 0):
        return f"{text} {object_name}."

    gt_names = gt_names[gt_names != object]

    if(len(gt_names) > 0):
        other_object_name = f"one {random.choice(gt_names)}"
        if(random_change):
            change_i = random.choice([True, False])
            if(change_i):
                temp = other_object_name
                other_object_name = object_name
                object_name = temp
        text = f"{text} {object_name} and {other_object_name}."
    else:
...
def text_location_l2(item, object="car", semantic=None):
    '''
        Key: text_location_l1
        Using The Training.
        Exmaple :
    ...
    if (not object in gt_names.tolist()):
        if(semantic is not None):
            indexes = check_semantic_object(semantic,scene=True)
            scene_i = random.choice(range(len(indexes)))
            return f"No {object}{suffix} in a scene with {class_name(scene_i)}."
        else:
            return f"There are no {object}{suffix}."
    ...
    if (len(all_descs) > 0):
        i = random.choice(range(len(other_names)))
        text = f"One {object} is to the {all_descs[i][0][1]} of one {other_names[i]}."
...
def text_orientation_star(item, object="car"):
    ...
    if (len(all_descs) > 0):
        text = f"One {object} is {orientation_text_rad(other_boxes[0], two_ori=True)}."
...
def downsample_text(infos, text, save_num=4000):
    ...
    if(len(li) == 0):
        return infos
    li = random.sample(li, len(li)-save_num)

    infos = np.delete(infos, li)
    return infos

def generation_object_text(
        root_path=None,
        desciption=None,
        object_name = "car",
):
    ...
    for i, info in enumerate(tqdm(infos, total=NUM, desc="Getting Text Decription : ")):
        ...
        # ----  数量: Two cars. ----
        text = text_quantity_l1(info, object_name, threshold=5)
        if (text is not None):
            item = {
        ...
        # ---- 位置: One car is to the right of a pedestrian. ----
        text = text_location_l2(info, object_name)
        if(text is not None):
            item = {
                "lidar_path": lidar_path,
                "text": text.replace("_",""),
        ...
        # ---- 组合: The scene contains one car and one pedestrian. ----
        text = text_contain_l1(info, object_name)
        if(text is not None):
            item = {
                "lidar_path": lidar_path,
                "text": text.replace("_",""),
        ...
        # ---- 朝向: One car is facing forward ----
        text = text_orientation_l1(info, object_name)
        if(text is not None):
            item = {
                "lidar_path": lidar_path,
                "text": text.replace("_",""),
        ...
        # # ---- 数量组合: Two cars and one trucks. ----
        # text = text_quantity_combination_l2(info, object_name)
        # if(text is not None):
        #     item = {
        #         "lidar_path": lidar_path,
        #         "text": text.replace("_",""),
        ...
        # # ---- 数量组合: Two cars and one trucks. ----
    print(len(item_list))
    item_list = downsample_text(item_list, "More than five cars.")
    item_list = downsample_text(item_list, "The scene contains one car and one pedestrian.")
    item_list = downsample_text(item_list, "One car is facing left.", save_num=3000)
    item_list = downsample_text(item_list, "One car is facing right.", save_num=3000)
    item_list = downsample_text(item_list, "One car is facing forward.", save_num=3000)
    item_list = downsample_text(item_list, "One car is facing backward.", save_num=3000)
    item_list = downsample_text(item_list, "Cars and pedestrians.", save_num=3000)
    print(len(item_list))
    ...
    check_text(item_list)


def check_text(infos=None):
    if(infos is None):
        infos = read_pkl(description="text.pkl")
    print(len(infos))
    text_num = {}
    for info in infos:
        text = info["text"]
...
if __name__ == '__main__':
    ...
    '''
        ...
        Scene : [10, 11, 12, 13, 14, 15]
        
        car的数量
        car的朝向
        car与其他对象
    '''
    # check_text("text_orientation_star")
    # get_num_everyclass()

    generation_object_text(desciption="text.pkl")
