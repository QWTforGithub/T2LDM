    ...
    def sample(
        ...
        if(self.upsampling):
            ...
            for batch in batches:
                ...
                points_batch = common.midpoint_interpolate(points_batch, up_rate=self.base_up_rate, only_FPS=True) # [1,C,N/2]
                points_batch = points_batch.permute(0,2,1).squeeze() # [1,N/2,C] -> [N/2,C]
                # ---- 下采样 ----

                # ---- sparse points -----
                # ---- RM ----
                sparse_batch = common.points_as_images_torch(points_batch, size=(self.sampling_shape[1], int(self.sampling_shape[2] * self.up_rate * self.base_up_rate))).permute(2,0,1) # [C,H,W]
                sparse_batch = sparse_batch.unsqueeze_(0)
                sparse_batch = common.convert_depth(sparse_batch)
                sparse_batch = common.normalize(sparse_batch)
                # ---- RM ----
                controls.append(sparse_batch)
                # ---- sparse points -----
                ...
                # ---- RM ----
                i_batch = common.points_as_images_torch(i_batch, size=(self.sampling_shape[1], int(self.sampling_shape[2] * self.base_up_rate * self.up_rate))).permute(2,0,1) # [C,H,W]
            ...
            x = self.randn(batch_size, *(self.sampling_shape[0], self.sampling_shape[1], int(self.sampling_shape[2] * self.base_up_rate * self.up_rate)), rng=rng, device=self.device)

            points = torch.cat(controls, dim=0)
            ...
            for batch in batches:
                ...
                # ---- dense points -----
                # ---- RM ----
                dense_batch = common.points_as_images_torch(points_batch, size=(self.sampling_shape[1], int(self.sampling_shape[2] * self.base_down_rate * self.down_rate))).permute(2,0,1) # [C,H,W]
                dense_batch = dense_batch.unsqueeze_(0)
                dense_batch = common.convert_depth(dense_batch)
                dense_batch = common.normalize(dense_batch)
                # ---- RM ----
                controls.append(dense_batch)
                # ---- dense points -----
                ...
                # ---- RM ----
                i_batch = common.points_as_images_torch(i_batch, size=(self.sampling_shape[1], int(self.sampling_shape[2] * self.base_down_rate * self.down_rate))).permute(2,0,1) # [C,H,W]
                i_batch = i_batch.unsqueeze_(0)
                i_batch = common.convert_depth(i_batch)
                i_batch = common.normalize(i_batch)
                # ---- RM ----
            ...
            x = self.randn(batch_size, *(self.sampling_shape[0], self.sampling_shape[1], int(self.sampling_shape[2] * self.base_down_rate * self.down_rate)), rng=rng, device=self.device)
        ...
        if(self.upsampling):
            return (x, points, noises) if return_noise else (x, points, None) # down -> up
