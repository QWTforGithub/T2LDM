...
from eval import build_model, VOXEL_SIZE, MODALITY2MODEL, MODAL2BATCHSIZE, DATASET_CONFIG, AGG_TYPE, NUM_SECTORS, \
    TYPE2DATASET, DATA_CONFIG
from eval.modules.chamfer3D.dist_chamfer_3D import chamfer_3DDist
from eval.modules.chamfer2D.dist_chamfer_2D import chamfer_2DDist
from eval.modules.emd.emd_module import emdModule
...
def compute_logits(
    ...
    assert dataset in ['nuscenes', 'kitti360']
    ...
    model = build_model(dataset, MODALITY2MODEL[modality], device='cuda')

    output = tuple()

    # ---- 抽取 reference 特征 ----
    reference_data = f"../{results_path}/{modality}_{dataset}_reference.pkl"
    if(os.path.exists(reference_data)):
        all_logits = common.read_pkl(reference_data)
        print(f"---- Loading : {reference_data} ----")
    else:
        all_logits_list = []
        progress_bar = tqdm(
            range(len(reference)),
        ...
        for i in range(math.ceil(len(reference) / bs)):
            batch = reference[i * bs:(i + 1) * bs]
            if is_voxel:
                ...
                batch = [preprocess_range(pcd, **dataset_config) for pcd in batch] # 将点云转换为range image,[B,64,1024]
        ...
        common.save_pkl(save_path=reference_data, infos=all_logits)
    ...
    # ---- 抽取 sample ----
    sample_data = f"{results_path}/{modality}_sample.pkl"
    if(os.path.exists(sample_data)):
        all_logits = common.read_pkl(sample_data)
        print(f"---- Loading : {sample_data} ----")
    else:
        all_logits_list = []
        ...
        for i in range(math.ceil(len(sample) / bs)):
            batch = sample[i * bs:(i + 1) * bs]
            if is_voxel:
                batch = [pcd2voxel(preprocess_pcd(pcd, **dataset_config)) for pcd in batch]
                batch = sparse_collate_fn(batch)
                batch = {k: v.cuda() if isinstance(v, (torch.Tensor, SparseTensor, PointTensor)) else v for k, v in
        ...
        #common.save_pkl(save_path=sample_data, infos=all_logits)
